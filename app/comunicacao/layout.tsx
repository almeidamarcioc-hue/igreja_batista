...
import ComunicacaoShell from '@/components/ComunicacaoShell'
...
export default async function ComunicacaoLayout({ children }: { children: React.ReactNode }) {
  ...
  if (modulos !== '*' && !modulos.split(',').map((m: string) => m.trim()).includes('comunicacao')) {
    redirect('/')
  }

  return <ComunicacaoShell>{children}</ComunicacaoShell>
}
