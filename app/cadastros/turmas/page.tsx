...
export default function TurmasPage() {
  ...
  return (
    <div>
      <PageHeader logo title="Turmas" subtitle={`${turmas.length} turma${turmas.length !== 1 ? 's' : ''} cadastrada${turmas.length !== 1 ? 's' : ''}`} />
          ...
          {filtered.map(t => (
            <div key={t.id} className="bg-white rounded-xl shadow-sm overflow-hidden">
              <div style={{ backgroundColor: '#1F2937', borderBottom: '2px solid #E07535' }} className="flex items-center gap-3 px-4 py-3">
                ...
                {!t.ativo && (
                  <span className="text-xs bg-gray-600 text-gray-200 px-2 py-0.5 rounded-full">Inativa</span>
                )}
              </div>
