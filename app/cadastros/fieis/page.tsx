...
export default function CadastroFieisPage() {
  ...
  const carregar = async (termo?: string) => {
    ...
    try {
      ...
      setErro('Erro ao carregar fiéis.')
    } finally {
      setLoading(false)
    }
  }
      ...
      <Modal isOpen={modalAberto} onClose={fecharModal} title={isNovo ? 'Novo Fiel' : 'Editar Fiel'}>
        <form onSubmit={handleSalvar} className="space-y-4">
          ...
          <div>
            ...
            <input type="text" value={fielEditando.nome ?? ''} onChange={(e) => setFielEditando((f) => ({ ...f, nome: e.target.value }))} required className={inputClass} style={inputStyle} onFocus={onFocus} onBlur={onBlur} />
          </div>
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-1">Telefone</label>
            <input type="tel" value={fielEditando.telefone ?? ''} onChange={(e) => setFielEditando((f) => ({ ...f, telefone: e.target.value }))} placeholder="(11) 99999-9999" className={inputClass} style={inputStyle} onFocus={onFocus} onBlur={onBlur} />
          </div>
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-1">Email</label>
            <input type="email" value={fielEditando.email ?? ''} onChange={(e) => setFielEditando((f) => ({ ...f, email: e.target.value }))} className={inputClass} style={inputStyle} onFocus={onFocus} onBlur={onBlur} />
          </div>
          <div>
          ...
          <div>
            ...
            <textarea value={fielEditando.observacoes ?? ''} onChange={(e) => setFielEditando((f) => ({ ...f, observacoes: e.target.value }))} rows={3} className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none resize-none" style={inputStyle} onFocus={onFocus} onBlur={onBlur} />
          </div>
