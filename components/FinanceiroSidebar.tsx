...
export default function FinanceiroSidebar({ open, onClose }: { open?: boolean; onClose?: () => void }) {
  ...
  const sidebar = (
    <aside style={{ backgroundColor: '#002347', borderRight: '2px solid #C5A059' }} className="flex flex-col h-full w-64">
      ...
      <div style={{ color: '#a0aec0' }} className="p-4 text-center text-xs">
        <div className="mb-2">{dd}/{mm}/{yyyy}</div>
        <div className="font-mono">{hh}:{min}</div>
      </div>
      ...
      <div className="px-4 pb-4">
        <Link
          ...
          className="w-full py-2 px-3 rounded-lg text-sm font-bold text-center transition-all duration-150 hover:opacity-90 block"
        >
          ← Voltar
        </Link>
      </div>
